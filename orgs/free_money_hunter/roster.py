"""The free-money-hunter org's roster, as structured data for the Hub's Org
view.

Unlike the in-repo studios, this cast and these gates live in the org's own
codebase (~/MoreSalamander/free-money-hunter). The descriptions are authored
...
_CAST: list[tuple[str, str, str]] = [
    ("Unclaimed-Property Scout", "scout", "Candidate specs found by searching official state/federal registries by name/address — untrusted by construction."),
    ("Settlement-Claims Scout", "scout", "Candidate specs for open class-action/settlement claim windows found via official sources."),
    ("Source Verification Agent", "verifier", "Confirms the claim resolves to an official government or court-administered domain."),
    ("Scam Detection Agent", "verifier", "Hunts fee-charging 'recovery service' scam reports — the org's standing red team."),
    ...
    ("Debate: Skeptic", "debate", "The danger case (thin for this domain — official-registry claims carry little residual risk) — its only teeth are scam reports the gate counts."),
    ("Debate: Strategist", "debate", "The claim-effort-vs-payout fit case against the user's profile — transcript only."),
    ("Portfolio Strategy Agent", "strategy", "A voiced strategy note over the deterministically selected mission."),
    ("Explainer", "coach", "Translates registry/claim jargon into plain English."),
]
...
_GATES: list[tuple[str, str, str]] = [
    ...
    ("scaffold:official_domain_allowlist", "hard", "at least one source on the official government/court/registry allowlist (subdomain-aware)"),
    ...
    ("scaffold:contract_verified", "hard", "not applicable to unclaimed-property/settlement claims; passes trivially when no on-chain claim is made"),
    ("scaffold:scam_report_evidence", "hard", "one credible scam report is a hard fail; unresolved reports fail closed; reports follow the claim across name variants"),
